...
from topeducation.services.free_recommendation_service import (
    FreeRecommendationError,
...
class filter_by_search(APIView):
    """
    Búsqueda rápida para el explorador.

    Estrategia:

    1. La consulta pesada de Certificaciones se mantiene liviana.
    2. Universidad / empresa / plataforma / tema / skills
       se resuelven primero a IDs.
    3. La consulta principal trabaja principalmente con FKs e IDs.
    4. Se obtienen únicamente los IDs de las primeras coincidencias.
    5. Solo después se hidratan las cards completas.
    """
    ...
    # Evita traer cientos de entidades auxiliares cuando el usuario
    # escribe algo muy genérico, por ejemplo "de".
    AUX_MATCH_LIMIT = 40
    ...
    def post(self, request):
        ...
        if len(query_string) < self.MIN_SEARCH_LENGTH:
            return Response(
                {
                    "results": [],
                    "count": 0,
        ...
        try:
            ...
            # Si el frontend ya envía IDs, no necesitamos
            # resolver nuevamente slugs.
            if selected_skill_ids:
                selected_skill_slugs = []

            # =====================================================
            # RESOLVER COINCIDENCIAS DEL TEXTO
            # =====================================================
            #
            # MUY IMPORTANTE:
            #
            # Antes la búsqueda principal hacía algo como:
            #
            # Certificaciones
            #   JOIN Universidad
            #   JOIN Empresa
            #   JOIN Plataforma
            #   JOIN Tema
            #   JOIN SkillsCertification
            #   JOIN Skills
            #
            # todo dentro de un OR gigante.
            #
            # Ahora primero buscamos IDs en cada tabla pequeña.
            #
            # =====================================================
            ...
            # =====================================================
            # SUBQUERY CERTIFICACIONES POR SKILL
            # =====================================================

            if search_skill_ids:
                search_skill_cert_ids = (
                    SkillsCertification.objects
                    .filter(
                        skill_id__in=
                        search_skill_ids
                ...
                search_skill_cert_ids = None

            # =====================================================
            # QUERYSET PRINCIPAL
            # =====================================================
            #
            # Sigue siendo deliberadamente LIVIANO.
            #
            # Aquí NO usamos:
            #
            # select_related
            ...
            # -----------------------------------------------------
            # TEMAS / HABILIDADES SELECCIONADAS
            # -----------------------------------------------------
            #
            # Reutilizamos exactamente la optimización
            # que ya hicimos para filter_by_tags.
            #
            # -----------------------------------------------------
            ...
            # =====================================================
            # FILTRO DEL TEXTO DE BÚSQUEDA
            # =====================================================
            #
            # A diferencia de la versión anterior, NO hacemos
            # JOIN a todas las relaciones dentro de este OR.
            #
            # =====================================================
            ...
            if search_topic_ids:
                search_q |= Q(
                    tema_certificacion_id__in=
                    search_topic_ids
                )

            if search_university_ids:
                search_q |= Q(
                    universidad_certificacion_id__in=
                    search_university_ids
                )

            if search_company_ids:
                search_q |= Q(
                    empresa_certificacion_id__in=
                    search_company_ids
                )

            if search_platform_ids:
                search_q |= Q(
                    plataforma_certificacion_id__in=
                    search_platform_ids
                )

            if search_skill_cert_ids is not None:
                search_q |= Q(
                    id__in=Subquery(
                        search_skill_cert_ids
                    )
            ...
            # =====================================================
            #
            # Este es el equivalente del patrón usado
            # por filter_by_tags.
            #
            # No hidratamos el queryset grande.
            #
            # =====================================================
            ...
            # =====================================================
            # HIDRATAR SOLO LAS CARDS NECESARIAS
            # =====================================================
            #
            # Reutilizamos el helper que ya existe para
            # filter_by_tags.
            #
            # =====================================================
...
def mx_magic_link_refresh(request):
    """
    Devuelve el Magic Link persistente entregado por México.

    Contrato vigente:
    - El Magic Link puede reutilizarse.
    - No expira dentro de la solución temporal actual.
    - Un cambio de paquete no requiere generar otro enlace.
    - No se genera USER_ACCESS_PROVISION por cada click.
    - No se registra el Magic Link en logs visibles.

    Se conserva el nombre `mx_magic_link_refresh`
    únicamente por compatibilidad con el frontend existente.
    """
    ...
    if (
        access_status not in {
            "",
            "ALLOWED",
        }
        or lifecycle_status in blocked_statuses
    ):
        return JsonResponse(
            {
                "ok": False,
                "error": "mx_access_not_allowed",
                "message": (
                    "Tu acceso a la plataforma "
                    "no está disponible actualmente."
                ),
                "access_status": (
                    access_status or None
                ),
                "lifecycle_status": (
                    lifecycle_status or None
                ),
            },
            status=403,
        )
    ...
    # =========================================================
    # FALLBACK A mx_response HISTÓRICO
    # =========================================================
    #
    # Útil para registros creados antes de que mx_magic_link
    # se persistiera directamente en una columna.
    # =========================================================
    ...
    if not magic_link:
        ...
        if isinstance(
            ...
            magic_link = str(
                ...
                or response_data.get("magicLink")
                or response_data.get("magic_link")
                or nested_response.get("magicLink")
                or nested_response.get("magic_link")
                or ""
            ).strip()

            # Si encontramos un link histórico válido,
            # persistirlo para las próximas consultas.
            if magic_link:
    ...
    if not magic_link:
        logger.warning(
            (
                "Magic Link persistente no encontrado. "
                "user=%s route=%s"
            ),
            user.id,
            route.id,
        ...
        return JsonResponse(
            {
                ...
                "message": (
                    "Tu acceso a Top Education México "
                    "todavía no tiene un enlace disponible."
                ),
    ...
    # =========================================================
    #
    # Nunca imprimir magic_link.
    # =========================================================

    logger.info(
        (
            "Magic Link persistente solicitado. "
            "user=%s route=%s mx_status=%s"
        ),
        user.id,
        route.id,
        mx_status,
    ...
    response = JsonResponse(
        {
            ...
            "data": {
                ...
                # Indica claramente al frontend
                # que estamos utilizando el enlace persistente.
                "reused": True,

                # Según contrato temporal vigente.
                "persistent": True,
                "expires": False,
            },
        },
...
class LearningRouteCreateView(APIView):
        ...
        *,
        ...
        if not isinstance(item, dict):
            return None, {
                ...
                "message": (
                    "Uno de los cursos recomendados no "
        ...
        if not id_interno:
            return None, {
                "error": (
                    "recommended_course_missing_internal_id"
                ),
                "message": (
                    "Uno de los cursos recomendados "
    ...
    def _normalize_recommended_courses(
        ...
        if not isinstance(courses, list):
            return None, {
                "error": (
                    "invalid_recommended_courses"
                ),
                "message": (
                    "recommended_courses debe ser "
                    "una lista."
                ),
            }
    ...
    def post(
        ...
        if not free_courses_raw:
            free_courses_raw = (
                self
                ._build_free_courses_from_recommended(
                    recommended_courses
                )
            )
...
class LearningRouteRecommendationsAPIView(APIView):
    ...
    def post(
        ...
        **kwargs,
        ...
        if topics:
            ...
            for topic_name in topics:
                ...
                text_filter |= (
                    ...
                    | Q(
                        skills__nombre__icontains=(
                            clean_topic
                        )
                    )
                    | Q(
                        skills__translate__icontains=(
                            clean_topic
                        )
                    )
                    | Q(
        ...
        presentation_values = list(
            self.LEVEL_PRESENTATION.values()
        )
        ...
        for index, course in enumerate(
            ...
            complete_serialized_courses.append(
                serialized
            )
        ...
        return Response(
            {
                ...
                "meta": {
                    ...
                    "total_complete_recommendations": (
                        len(
                            complete_serialized_courses
                        )
                    ),

                    "total_visual_recommendations": (
                        len(
                            serialized_courses
                        )
                    ),
                    ...
                    "complete_internal_ids": [
                        ...
                        for item
                        in complete_serialized_courses
                    ],
